...
**Copy that path and branch name** — you'll need both at merge time.
...
### 5. Merge the FAQ worktree into the new style (Terminal A)

```bash
# from the main checkout
git worktree list                                  # confirm both exist
git fetch --all                                    # noop locally but good habit
...
If you'd **rejected** the experiment instead:
...
Merge the winners, bin the rest:
